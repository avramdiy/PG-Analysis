...
@app.route('/monthly_high_boxplot')
def monthly_high_boxplot():
    file_path = r"C:\Users\avram\OneDrive\Desktop\TRG Week 30\pg.us.txt"
    try:
        # Load and filter the DataFrame
        df = pd.read_csv(file_path, sep=",", engine="python", parse_dates=['Date'], infer_datetime_format=True)
        ...
        # Plot the box plot for "High" prices
        plt.figure(figsize=(12, 6))
        df.boxplot(column='High', by='Month', patch_artist=True, grid=False, color=dict(boxes='blue', whiskers='black'))
        plt.title('Monthly High Price Distribution (1970-1979)', fontsize=16)
        plt.suptitle('')
